...
-- -----------------------------------------------------
-- Table `emiliabd`.`produto`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `emiliabd`.`produto` ;

CREATE  TABLE IF NOT EXISTS `emiliabd`.`produto` (
...
-- -----------------------------------------------------
-- Table `emiliabd`.`preco`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `emiliabd`.`preco` ;

CREATE  TABLE IF NOT EXISTS `emiliabd`.`preco` (
  `CD_PRECO` INT UNSIGNED NOT NULL AUTO_INCREMENT ,
  `NR_PRECO` DECIMAL(10,2) NULL ,
...
CREATE  TABLE IF NOT EXISTS `emiliabd`.`item` (
  ...
  `CD_PRECO` INT UNSIGNED NOT NULL ,
  PRIMARY KEY (`CD_ITEM`) ,
  INDEX `fk_item_preco1_idx` (`CD_PRECO` ASC) ,
  INDEX `fk_item_produto1_idx` (`CD_PRODUTO` ASC) ,
  CONSTRAINT `fk_item_preco1`
    FOREIGN KEY (`CD_PRECO` )
    REFERENCES `emiliabd`.`preco` (`CD_PRECO` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_item_produto1`
    FOREIGN KEY (`CD_PRODUTO` )
    REFERENCES `emiliabd`.`produto` (`CD_PRODUTO` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
